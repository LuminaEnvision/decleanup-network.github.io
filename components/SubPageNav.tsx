...
export default function SubPageNav() {
    ...
    return (
        <nav className="fixed top-0 w-full z-50 bg-[#0A0A0A]/95 backdrop-blur-md border-b border-black/5 dark:border-white/5">
            <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
                ...
                {/* Desktop Links */}
                <div className="hidden lg:flex items-center gap-6 text-[10px] xl:gap-8 xl:text-[11px] text-gray-800 dark:text-gray-200 font-medium font-mono uppercase tracking-widest">
                    {links.map((link) => (
            ...
            {/* Mobile Menu Overlay */}
            {isMobileMenuOpen && (
                <div className="lg:hidden absolute top-16 left-0 w-full bg-[#0A0A0A]/98 border-b border-black/10 dark:border-white/10 py-6 px-6 flex flex-col gap-4 animate-in fade-in slide-in-from-top-4 duration-300">
                    {links.map((link) => (
                        <Link
                            key={link.href}
                            href={link.href}
                            className="text-sm text-gray-700 dark:text-gray-300 hover:text-[#FAFF00] font-mono uppercase tracking-widest py-2"
